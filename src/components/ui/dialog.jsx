...
export function DialogContent({ className = "", children }) {
  ...
  const content = (
    <div className="fixed inset-0 z-[30000] flex items-center justify-center bg-black/55 p-4" onMouseDown={() => ctx.onOpenChange(false)}>
      <div
        ref={revealRef}
        className={`app-dialog-surface ui-reveal-target relative max-h-[92vh] w-full max-w-lg overflow-y-auto p-5  ${className}`.trim()}
        style={{ "--reveal-radius": "30px" }}
        onMouseDown={(event) => event.stopPropagation()}
      >
        <button
          type="button"
          className="app-dialog-close absolute right-5 top-5 z-10 min-h-9 rounded-xl border px-3.5 py-1.5 text-sm font-semibold  transition-colors"
