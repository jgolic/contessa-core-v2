...
import { canUseDemoEditing, getRuntimePublicAppUrl } from "./lib/runtime_config.mjs";
...
function resolveExplicitWorkspaceVesselName(workspace = {}) {
  return String(
    workspace.routePlanning?.vesselProfile?.vesselName ||
    workspace.routePlan?.vesselProfile?.vesselName ||
    workspace.route?.vesselProfile?.vesselName ||
    workspace.vesselProfile?.vesselName ||
    ""
  ).trim();
}
...
function buildContessaWorkspace(name = "M/Y Contessa") {
  ...
  const tasks = [
    {
      ...
      comments: [{ id: "CON-COM-001", text: "Possible coating issue flagged during hull inspection.", by: "Oliver Reed", at: demoTimestamp(8) }],
    },
    {
      id: "CON-TASK-002",
  ...
  const history = [
    { id: "CON-HIS-001", at: demoTimestamp(0), section: "Maintenance", action: "Coating issue flagged", detail: "Oliver flagged possible coating issue in bow thruster tunnel." },
...
function buildOctopussyWorkspace(name = "M/Y Octopussy") {
  ...
  const tasks = [
    ...
    {
      id: "OCT-TASK-006",
      name: "Stage boarding ladder safety check",
      area: "Swim Platform",
      department: "Deck",
      assignee: "Leo Grant",
      status: "pending",
      ...
      approvalStatus: "approved",
      notes: "Confirm boarding ladder locking points and guest handhold setup before arrival.",
    },
  ].map((task) => normalizeTask(task));
  ...
  const history = [
    { id: "OCT-HIS-001", at: demoTimestamp(0), section: "Maintenance", action: "Hydraulic residue reported", detail: "Adrian reported hydraulic residue near tender davit." },
...
export function createFleetVesselWorkspace({
  ...
  const explicitProfileName = resolveExplicitWorkspaceVesselName(workspace);
  const effectiveProfileName = explicitProfileName || displayName;
  const normalizedProfile = normalizeVesselProfile({
    ...(presetWorkspace.vesselProfile || normalizeVesselProfile({ vesselName: effectiveProfileName })),
    ...(workspace.vesselProfile || {}),
    vesselName: effectiveProfileName,
  });
  const normalizedRouteProfile = normalizeVesselProfile({
    ...(presetWorkspace.routePlanning?.vesselProfile || normalizedProfile),
    ...(workspace.routePlanning?.vesselProfile || {}),
    vesselName: effectiveProfileName,
  });
  const normalizedWorkspace = normalizeWorkspaceState({
    ...
    routePlanning: {
      ...
      vesselProfile: normalizedRouteProfile,
    },
  });
...
export function normalizeFleetVessel(vessel = {}, fallbackId = DEFAULT_FLEET_VESSEL_ID) {
  ...
  const name = vessel.name || fallbackName;
  const explicitProfileName = String(
    vessel.routePlanning?.vesselProfile?.vesselName ||
    vessel.routePlan?.vesselProfile?.vesselName ||
    vessel.route?.vesselProfile?.vesselName ||
    vessel.vesselProfile?.vesselName ||
    ""
  ).trim();
  const effectiveProfileName = explicitProfileName || name;
  ...
  const normalizedWorkspace = normalizeWorkspaceState({
    ...vessel,
    vesselProfile: {
      ...(vessel.vesselProfile || createFallbackVesselProfile(effectiveProfileName)),
      vesselName: effectiveProfileName,
    },
    routePlanning: {
      ...(vessel.routePlanning || vessel.routePlan || vessel.route || createEmptyRoutePlanningState()),
      vesselProfile: {
        ...(vessel.routePlanning?.vesselProfile || vessel.routePlan?.vesselProfile || vessel.route?.vesselProfile || vessel.vesselProfile || createFallbackVesselProfile(effectiveProfileName)),
        vesselName: effectiveProfileName,
      },
    },
  });
...
export function buildPendingApprovalItems({ tasks = [], boatExpenses = [], crewExpenses = [] } = {}) {
  ...
  return [
    ...pendingBoatItems.map((item) => ({
      ...
      requestedAt: item.requestedAt || item.createdAt || item.updatedAt || null,
    ...
    ...tasks
      ...
      .map((task) => ({
        ...
        requestedAt: task.requestedAt || task.createdAt || task.updatedAt || null,
...
export function createEmptyAppState(overrides = {}) {
  ...
  return {
    ...
    currentRole: "captain",
    appMode: canUseDemoEditing() ? "editor" : "view",
...
export function createPersistedAppState(state) {
  ...
  return {
    ...
    appMode: canUseDemoEditing() && state.appMode === "editor" ? "editor" : "view",
    activeVesselId,
    vessels: fleet,
    history: activeVessel.history,
    declinedTasks: activeVessel.declinedTasks,
...
export function getInitialAppState() {
  ...
  try {
    ...
    return isEssentiallyEmpty
      ? fallback
      : {
        ...normalized,
        appMode: canUseDemoEditing() && normalized.appMode === "editor" ? "editor" : "view",
      };
  } catch {
    return fallback;
  }
