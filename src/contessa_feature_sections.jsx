            ...
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
              {fleetVessels.map((vessel) => {
                ...
                const vesselToneClass = darkMode
                  ? "border-white/12 bg-slate-900/94 shadow-[0_20px_52px_rgba(0,0,0,0.26)]"
                  : "border-slate-200/90 bg-slate-50/95 shadow-[0_20px_52px_rgba(15,23,42,0.08)]";
                ...
                return (
                  <div key={vessel.id} className={`relative flex h-full flex-col gap-2.5 overflow-hidden rounded-[22px] border p-4 backdrop-blur-xl transition-all duration-200 hover:-translate-y-0.5 ${vesselToneClass} ${isActive ? (darkMode ? "ring-1 ring-cyan-300/25" : "ring-1 ring-slate-300/80") : ""}`}>
                    <div className={`pointer-events-none absolute inset-x-0 top-0 h-1 ${isActive ? (darkMode ? "bg-cyan-300/55" : "bg-slate-700/35") : (darkMode ? "bg-white/8" : "bg-slate-300/70")}`} />
                    <div className={`pointer-events-none absolute right-[-42px] top-[-52px] h-32 w-32 rounded-full blur-3xl ${darkMode ? "bg-cyan-300/8" : "bg-slate-300/35"}`} />
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {fleetMetricCards.map((metric) => (
                        <div key={metric.label} className={`group rounded-2xl border p-3 shadow-[inset_0_1px_0_rgba(255,255,255,0.45)] transition-colors duration-200 ${metricToneClass}`}>
