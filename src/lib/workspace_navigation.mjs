...
export function parseWorkspaceView(search = "") {
  ...
  const view = String(params.get(WORKSPACE_VIEW_PARAM) || "fleet").toLowerCase();
  return VIEW_CONFIG[view] ? { view, ...VIEW_CONFIG[view] } : { view: "fleet", ...VIEW_CONFIG.fleet };
}
...
export function updateWorkspaceViewUrl(currentUrl, moduleName, options = {}) {
  ...
  if (view === "fleet") url.searchParams.delete(WORKSPACE_VIEW_PARAM);
